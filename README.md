...
- **Optional (viz)**: Matplotlib (`pip install pyegm[viz]`)  
- **Optional (acceleration)**: **User-installed** PyTorch + CUDA for GPU scoring (falls back to CPU)

**API**: `fit`, `partial_fit`, `predict`, `score`, `save`, `load`, `get_fitted_params`, `visualize_explosion`
...
pip install "pyegm[viz]"
```
Core requirements: `numpy>=1.21`, `scikit-learn>=1.0`  
Optional (viz): `matplotlib>=3.5`, `pillow>=9.0`
...
- Visualization is optional and depends on Matplotlib.
...
- **可选（可视化）**：Matplotlib（`pip install pyegm[viz]`）  
...
pip install "pyegm[viz]"           # 可选：可视化
```
核心依赖：`numpy>=1.21`, `scikit-learn>=1.0`  
可视化：`matplotlib>=3.5`, `pillow>=9.0`
...
## 说明
- 不依赖 ANN/HNSW。  
- 可视化为可选功能；未安装 Matplotlib 时不影响主功能。
