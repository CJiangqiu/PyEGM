...
class PyEGM(BaseEstimator, ClassifierMixin):
    ...
    def visualize_explosion(
        ...
        projection: Literal["2d", "3d", "pca2d", "pca3d"] = "pca3d",
        per_class_points: int = 400,
        save_path: Optional[str] = None,
        show: bool = True,
        title: str = "PyEGM — Prototypes and Arrival Centers",
    ) -> None:
        """
        Visualize class prototypes and arrival centers.

        Parameters
        ----------
        X, y : ndarray or None
            Optional data to overlay; at most `per_class_points` per class
            are shown for clarity.

        projection : {"2d","3d","pca2d","pca3d"}, default="pca3d"
            Coordinates used for plotting. PCA projections are applied
            on the prepared feature space.

        per_class_points : int, default=400
            Maximum number of sample points per class when overlaying data.

        save_path : str or None
            When provided, the figure is saved to this path.
        ...
        Notes
        -----
        This function depends on Matplotlib. It does not require GPU.
        """
        if self._classes is None or self._mean is None or self._std is None or self._scale is None:
            raise RuntimeError("Model must be fitted before visualization.")

        import matplotlib.pyplot as plt

        use_3d = projection in ("3d", "pca3d")
        want_pca = projection in ("pca2d", "pca3d")

        items: List[np.ndarray] = []
        tags: List[Tuple[str, int, int]] = []  # ("data"/"proto"/"arrival", class_idx, shell_idx)
        colors: List[Any] = []

        C = self._classes.shape[0]
        cmap = plt.cm.get_cmap("tab20", max(C, 1))

        def color_for_cls(ci: int):
            return cmap(ci / max(C - 1, 1)) if C > 1 else cmap(0.0)

        # data overlay (faint)
        if X is not None and y is not None:
            X = np.asarray(X, dtype=np.float32)
            y = np.asarray(y, dtype=np.int64)
            for ci, cls in enumerate(self._classes.tolist()):
                idx = np.where(y == cls)[0]
                if len(idx) == 0:
                    continue
                if len(idx) > per_class_points:
                    rng = np.random.default_rng(0)
                    idx = rng.choice(idx, size=per_class_points, replace=False)
                Xi = X[idx]
                items.append(Xi)
                tags.extend([("data", ci, -1)] * Xi.shape[0])
                colors.extend([color_for_cls(ci)] * Xi.shape[0])

        # prototypes
        items.append(self._mean)
        tags.extend([("proto", i, -1) for i in range(C)])
        colors.extend([color_for_cls(i) for i in range(C)])

        # arrival centers
        for ci in range(C):
            shells = self._centers_for_class(ci)
            for s_idx, Z in enumerate(shells):
                items.append(Z)
                tags.extend([("arrival", ci, s_idx)] * Z.shape[0])
                colors.extend([color_for_cls(ci)] * Z.shape[0])

        Xall = np.vstack(items) if items else self._mean.copy()
        Xsrc = _normalize_rows(Xall) if (self.cfg.metric == "cos" and self.cfg.normalize) else Xall

        K = 3 if use_3d else 2
        if want_pca:
            try:
                from sklearn.decomposition import PCA
                emb = PCA(n_components=min(K, Xsrc.shape[1]), random_state=0).fit_transform(Xsrc)
                if emb.shape[1] < K:
                    emb = np.pad(emb, ((0, 0), (0, K - emb.shape[1])), mode="constant")
            except Exception:
                emb = Xsrc[:, :K] if Xsrc.shape[1] >= K else np.pad(Xsrc, ((0, 0), (0, K - Xsrc.shape[1])))
        else:
            emb = Xsrc[:, :K] if Xsrc.shape[1] >= K else np.pad(Xsrc, ((0, 0), (0, K - Xsrc.shape[1])))

        counts = [arr.shape[0] for arr in items]
        offs = np.cumsum([0] + counts)

        def segment(k: int) -> np.ndarray:
            return emb[offs[k]:offs[k + 1]]
        ...
        for k in range(len(items)):
            if tags[ptr][0] != "data":
                ...
                continue
            pts = segment(k)
            ax.scatter(*(pts.T), s=6, alpha=0.18, c=[colors[ptr]] * len(pts), edgecolors="none")
            ptr += counts[k]

        # arrival centers
        ptr = 0
        for k in range(len(items)):
            if tags[ptr][0] != "arrival":
                ptr += counts[k]
                continue
            pts = segment(k)
            ax.scatter(*(pts.T), s=16, alpha=0.65, c=[colors[ptr]] * len(pts), marker=".", edgecolors="none")
            ptr += counts[k]
        ...
        for k in range(len(items)):
            if tags[ptr][0] != "proto":
                ...
                continue
            pts = segment(k)
            ax.scatter(*(pts.T), s=120, alpha=0.95, c=[colors[ptr]] * len(pts), marker="*",
                       edgecolors="black", linewidths=0.6, label=f"class {int(self._classes[tags[ptr][1]])}")
            ptr += counts[k]
